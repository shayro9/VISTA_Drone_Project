...
for epoch in range(EPOCHS):
    ...
    subprocess.run(move_images_cmd, shell=True)
    result = subprocess.run(["ssh", SERVER_BASE, "cd ~/drone/DPVO && srun -c 2 --gres=gpu:1 ~/drone/DPVO/run_no_attack.sh path_0"],
                            text=True,
                            capture_output=True)
...
for i in range(PATHS):
	...
	apply_noise(image_dir, noise_path)
	subprocess.run(move_images_cmd, shell=True)
	subprocess.run(["ssh", SERVER_BASE, f"cd ~/drone/DPVO && srun -c 2 --gres=gpu:1 ~/drone/DPVO/plot_traj.sh path_{i}"])
